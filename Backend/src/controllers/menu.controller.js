...
const { MESSAGES, HTTP_STATUS, MENU_CATEGORIES } = require('../config/constants');
...
// ✅ Filter by category (matches menu.html category-pill)
if (category && category !== 'all') {
filter.category = category;
}

// ✅ Filter by availability (matches menu.html status-stock)
...
// ✅ Search (matches menu.html search-input)
if (search) {
filter.$or = [
...
// ✅ Format response for frontend (matches menu.html expectations)
...
* @access  Public
*
* Frontend: food-details.html → Load single food details
...
// ✅ Validate name has both languages
if (!name.en || !name.am) {
return
...
if (name) {
item.name = {
en: name.en?.trim() || item.name.en,
am: name.am?.trim() || item.name.am
};
}
...
en: description.en || item.description.en,
am: description.am || item.description.am
};
}
...
* Expected Body: { available: true/false }
...
* Response: { totalItems, availableItems, outOfStockItems, totalCategories }
*/
