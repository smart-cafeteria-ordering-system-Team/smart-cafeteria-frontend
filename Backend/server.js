...
const { MONGODB_URI, validateEnv } = require("./src/config/env");
...
const allowedOrigins = process.env.CORS_ORIGIN
  ...
  : [frontendUrl, "http://localhost:3000", "http://127.0.0.1:5500"];

if (isProductionMode && !process.env.CORS_ORIGIN) {
  console.warn(`[CORS] Production warning: CORS_ORIGIN not set. Using FRONTEND_URL "${frontendUrl}".`);
}
      ...
      if (!isProductionMode) {
        try {
          ...
          if (
            ...
            console.log(
              `[CORS] ✓ Allowed: "${origin}" (localhost in development)`,
            );
            callback(null, true);
            return;
          }
...
// Health check endpoints (Render health check path: /api/health)
// Reports genuine application + database state. The server only starts
// listening after a successful DB connection, and this uses the live
// mongoose.connection.readyState so callers see real status.
app.get("/health", (req, res) => {
  const dbState = mongoose.connection.readyState; // 0=disconnected 1=connected 2=connecting 3=disconnecting
  const dbConnected = dbState === 1;
  res.status(dbConnected ? 200 : 503).json({
    success: dbConnected,
    service: "smart-cafeteria-backend",
    status: dbConnected ? "healthy" : "degraded",
    database: dbConnected ? "connected" : `disconnected (readyState ${dbState})`,
...
const start = async () => {
  // Fail clearly and fast when critical configuration is missing in production.
  validateEnv();
  ...
  try {
    ...
    console.warn(
      "[Server] Warning: Could not initialize settings:",
      error.message,
    );
  }
  ...
  const shutdown = (signal) => {
    ...
    server.close(async () => {
      try {
        ...
        console.error("[Server] Error while disconnecting MongoDB:", err.message);
      }
...
// Do not hide real production errors: crash loudly instead of continuing broken.
