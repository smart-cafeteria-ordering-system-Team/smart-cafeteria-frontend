...
document.addEventListener("DOMContentLoaded", function () {
            ...
            return `
                <div class="cart-item-card">
                    <div class="cart-item-info">
                        <div class="cart-item-title">${escapeHTML(item.name || "Food Item")}</div>
                        <div class="cart-item-price">${price.toFixed(2)} ETB</div>
                    </div>
    ...
    function updateCart() {
        ...
        if (navbarCartCount) navbarCartCount.textContent = totalQuantity;
        if (itemsCartCount) itemsCartCount.textContent = totalQuantity;
        if (subtotalElement) subtotalElement.textContent = subtotal.toFixed(2);
        if (serviceFeeElement) serviceFeeElement.textContent = serviceFee.toFixed(2);
        if (totalElement) totalElement.textContent = total.toFixed(2);
        if (clearCartBtn) clearCartBtn.style.display = cart.length > 0 ? "inline-flex" : "none";
